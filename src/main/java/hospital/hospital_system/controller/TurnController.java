...
public class TurnController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> updateTurn(@PathVariable Integer id,
                                        @RequestBody TurnDTO turnDTO) {
        ApiResult<TurnDTO> apiResult = turnService.update(id, turnDTO);
        return ResponseEntity.ok(apiResult);
    }

    @CheckAuth(permissions = PermissionEnum.DELETE_TURN)
    @DeleteMapping("/{id}")
    public ResponseEntity<?> deleteTurn(@PathVariable int id) {
        ApiResult<TurnDTO> apiResult = turnService.delete(id);
        return ResponseEntity.ok(apiResult);
    }
